    ...
    private void initMergeByID(boolean union) {
        ...
        //this is not the most efficient way, but it works well with other code I use and heck it is plenty fast
        {
            ...
            for (int i = 0; i < ids.length; i++) {
                ids[i] = intersectionVector.get(i);
            }
        } else {
            ids = new Identifier[idVector.size()];
            for (int i = 0; i < ids.length; i++) {
                ids[i] = idVector.get(i);
            }
        }
...
//                for(int i=0; i<ids.length; i++) {ids[i]=(Identifier)idVector.get(i);}
    ...
    public char getData(int seq, int site) {
        if (mergeByID) {
            if (sequenceIndex[alignmentIndex[site]][seq] == -1)
                return '?';
            else
    ...
    public int getChromosome(int site) {
        return alignmentList[alignmentIndex[site]].getChromosome(siteIndex[site]);
    }
    ...
    public int getLocusPosition(int site) {
        return alignmentList[alignmentIndex[site]].getLocusPosition(siteIndex[site]);
    }
    ...
    public String getLocusName(int site) {
        return alignmentList[alignmentIndex[site]].getLocusName(siteIndex[site]);
    }
