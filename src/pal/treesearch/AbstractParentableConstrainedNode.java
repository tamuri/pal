...
public abstract class AbstractParentableConstrainedNode implements ParentableConstrainedNode {
    ...
    public double getMinOriginalDescendentLeafHeight() {
        return minOriginalDescendentLeafHeight_;
    }

    public double getMaxOriginalDescendentLeafHeight() {
        return maxOriginalDescendentLeafHeight_;
    }
...
//	} //End of getMinimumDescendentRelativeHeight()
    ...
    protected final PatternInfo getRightChildPatternInfo(GeneralConstructionTool tool) {
        return rightChild_.getDescendentPatternInfo(tool);
    }

    protected final PatternInfo getLeftChildPatternInfo(GeneralConstructionTool tool) {
        return leftChild_.getDescendentPatternInfo(tool);
    }
...
//		System.out.println("before:"+before+", after:"+nodeHeight_);
    ...
    protected final Node buildLeftDecendentPALNodeBase() {
        return leftChild_.buildDescendentPALNodeBase();
    }

    protected final Node buildRightDecendentPALNodeBase() {
        return rightChild_.buildDescendentPALNodeBase();
    }
...
// =-=-=-==--=-=-==-=--=-=-=-=-=-=-=-==--=-==--=-=-=-=-=-==-=--=-=-=-=-==-=-=--=-=-==-=-=-=-=--=-=-==-=--==-=--=-=-==--=
    ...
    public final void getSubTreeComponents(ArrayList store, Class componentType) {
        ...
        leftChild_.getSubTreeComponents(store, componentType);
        rightChild_.getSubTreeComponents(store, componentType);
    }

    public final void getAllComponents(ArrayList store, Class componentType) {
        getSubTreeComponents(store, componentType);
    ...
    public void getNonSubTreeOfChildComponents(ArrayList store, Class componentType, ConstrainedNode childCaller) {
        ...
        getNonSubTreeComponents(store, componentType);
        if (leftChild_ == childCaller) {
            rightChild_.getSubTreeComponents(store, componentType);
        } else if (rightChild_ == childCaller) {
            leftChild_.getSubTreeComponents(store, componentType);
        } else {
