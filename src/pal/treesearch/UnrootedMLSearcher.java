...
// =--=-==-=---==--=-==--=-=-=-=-==-=-=--=-==-=-=-=--==-=-=-=-=--==-=-=-=-=-=-
    private static final class Workspace {
        ...
        public final double[] getEndStateProbabilityStore() {
            return endStateProbabilityStore_;
        }
...
// =--=-==-=--=-==--=-==--=-=-=-=-==-=-=--=-==-=-=-=--==-=-=-=-=--==-=-=-=-=-=-
    private static final class InternalNode implements UNode {
        ...
        public void instruct(UnrootedTreeInterface.UNode node, Connection callingConnection) {
            if (annotation_ != null) {
                node.setAnnotation(annotation_);
            }
        ...
        private int getCallerIndex(Connection caller) {
            ...
            if (caller == connections_[0]) {
                return 0;
            }
            if (caller == connections_[1]) {
                return 1;
            }
            if (caller == connections_[2]) {
                return 2;
            }
...
// -=-=-=-=-=-=-==--=-=-=-=-=-=-==--=-=-==-=--=-=-=-==-=--=-=-=-=-=-=-==-=-=-=-
    ...
    private static final class LeafNode implements UNode {
        ...
        public int rebuildPattern(ConstructionTool tool) {
            return parentConnection_.getOther(this).redirectRebuildPattern(tool);
        }
...
//			final UNode baseLeftNode = original.leftNode_.createAlteredCopy(attachmentPoint,newSubtree,original,this,tool);
        ...
        public String[] getLeftLeafNames() {
            return getLeafNames(leftNode_);
        }

        public String[] getRightLeafNames() {
            return getLeafNames(rightNode_);
        }
        ...
        public boolean isLeafBranch(String leafLabel) {
            if (leftNode_.isLeaf()) {
                if (leftNode_.hasLabel(leafLabel)) {
                    return true;
                }
            }
            if (rightNode_.isLeaf()) {
                if (rightNode_.hasLabel(leafLabel)) {
                    return true;
                }
            }
            return false;
        ...
        public final void mark() {
            ...
            this.markLeftNode_ = leftNode_;
            this.markRightNode_ = rightNode_;
        }
        ...
        public final PatternInfo getLeftPatternInfo() {
            return leftNode_.getPatternInfo(this);
        }
        ...
        public final void instruct(UnrootedTreeInterface.UBranch branch, UNode callingNode) {
            ...
            if (annotation_ != null) {
                branch.setAnnotation(annotation_);
            }
...
// =--=-=-=-==-=-
    ...
    private static final class ConstructionTool {
        ...
        public boolean hasSequences() {
            return sequences_ != null && sequences_.length > 0;
        }
