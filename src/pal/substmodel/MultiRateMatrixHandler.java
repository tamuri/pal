...
public class MultiRateMatrixHandler implements NeoParameterized, java.io.Serializable {
        ...
        for (int i = 0; i < dimension - 1; i++) {
            for (int j = i + 1; j < dimension; j++) {
                if (frequencies[i] == frequencies[j]) {
                    frequencies[i] += MINFDIFF;
                    frequencies[j] -= MINFDIFF;
                }
            }
        }
    }
    ...
    public final double getSubstitutionClassLowerLimit() {
        return SUBSTITUTION_CLASS_LOWER_LIMIT;
    }
    ...
    public final double getSubstitutionClassDefaultValue() {
        return SUBSTITUTION_CLASS_DEFAULT_VALUE;
    }

    public double getLowerLimit(int n) {
        return rateMatrices_[baseLookup_[n]].getRateParameterLowerBound(rateParameterIndexLookup_[n]);
    }

    public double getUpperLimit(int n) {
        return rateMatrices_[baseLookup_[n]].getRateParameterUpperBound(rateParameterIndexLookup_[n]);
    }

    public double getDefaultValue(int n) {
        return defaultRateParameters_[n];
    }

    public int getNumberOfParameters() {
        return rateParameters_.length;
    }
