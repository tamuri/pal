...
public class YangCodonModel extends CodonModel implements Serializable, XMLConstants {
    ...
    public void setParameterSE(double paramSE, int n) {
        switch (n) {
            case KAPPA_PARAMETER: {
                kappaSE = paramSE;
                break;
            }
            case OMEGA_PARAMETER: {
                omegaSE = paramSE;
                break;
            }
            default:
    ...
    public final void setKappa(double value) {
        setParameter(value, KAPPA_PARAMETER);
    }

    public final void setOmega(double value) {
        setParameter(value, OMEGA_PARAMETER);
    }

    public double getLowerLimit(int n) {
        return n == KAPPA_PARAMETER ? MINIMUM_KAPPA : MINIMUM_OMEGA;
    }

    public double getUpperLimit(int n) {
        return n == KAPPA_PARAMETER ? MAXIMUM_KAPPA : MAXIMUM_OMEGA;
    }

    public double getDefaultValue(int n) {
        return n == KAPPA_PARAMETER ? DEFAULT_KAPPA : DEFAULT_OMEGA;
    }
    ...
    public static class SimplePositiveSelection extends PalObjectListener.EventGenerator implements SubstitutionModel {
        ...
        private void rebuild() {
            ...
            if (total == 0) {
                ...
                probabilities_[1] = 0.33;
                probabilities_[2] = 0.34;
            } else {
                probabilities_[0] = p0_ / total;
                probabilities_[1] = p1_ / total;
                probabilities_[2] = p2_ / total;
            }
...
// -=-=-=-==--=-=-=-=-==--==--=-=-=-=-==--==--==-=-==--=-=-=-=-=-==================-=-=-=-=-==--=-==--==-=--=-=-=-=-=-=
    ...
    public static class SimpleNeutralSelection extends PalObjectListener.EventGenerator implements SubstitutionModel {
        ...
        public void getTransitionProbabilities(double branchLength, double[][][] tableStore) {
            check();
            baseMatrixes_[0].setDistance(branchLength);
            baseMatrixes_[0].getTransitionProbabilities(tableStore[0]);
            baseMatrixes_[1].setDistance(branchLength);
            baseMatrixes_[1].getTransitionProbabilities(tableStore[1]);
        }
        ...
        public void getTransitionProbabilitiesTranspose(double branchLength, double[][][] tableStore) {
            check();
            baseMatrixes_[0].setDistanceTranspose(branchLength);
            baseMatrixes_[0].getTransitionProbabilities(tableStore[0]);
            baseMatrixes_[1].setDistanceTranspose(branchLength);
            baseMatrixes_[1].getTransitionProbabilities(tableStore[1]);
